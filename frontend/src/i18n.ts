...
const dictionaries: Record<Locale, Dictionary> = {
  zh: {
    ...
    'home.form.submittingTitle': '正在计算候选方案',
    'home.form.submittingBody':
      '当前请求会在返回前完成候选生成、物理仿真和决策摘要，请稍候，不要重复提交。',
    'home.form.submittingHint': '接口返回后会自动进入结果页。',
    'home.form.submittingStep.generate': '生成候选膜层',
    'home.form.submittingStep.simulate': '执行薄膜仿真与局部优化',
    'home.form.submittingStep.decision': '整理 AI 决策建议',
    ...
    'workspace.loadingTitle': '正在读取方案结果',
    'workspace.loadingNote': '正在同步候选方案、AI 决策摘要和 artifact。',
    'decision.mode.llm': 'LLM 辅助',
    'decision.mode.heuristic': '规则回退',
    'decision.confidence.high': '高置信',
    'decision.confidence.medium': '中等置信',
    'decision.confidence.low': '低置信',
    'decision.explainer.title': 'AI 决策在做什么',
    'decision.explainer.body': '当前 AI 决策层负责把计算结果整理成可评审建议，不直接替代候选生成和物理仿真。',
    'decision.role.recommend': '基于已生成候选，给出当前建议优先评审的方案：{candidateId}。',
    'decision.role.scope': '输出内容包括推荐标题、摘要、推荐理由、风险提醒和下一步动作。',
    'decision.role.limit': '它不直接训练模型，也不直接计算膜层厚度；这些仍由逆向设计和薄膜仿真完成。',
    'decision.role.llm': '当前已启用 LLM 辅助，模型为 {model}。',
    'decision.role.fallback': '当前未使用可用 LLM 响应，系统已回退到本地规则判断。',
    'decision.inputs.title': '决策层当前使用的输入',
    'decision.input.request': '基础输入包括需求描述、目标颜色、入射角和偏振态。',
    'decision.input.candidates': '会参考前 {count} 个候选方案的结构参数、指标和说明。',
    'decision.input.constraints': '还会参考最多 {count} 条制造约束或风险提示。',
    'decision.input.llmPrompt': '在 LLM 模式下，这些信息会被整理成结构化上下文并发送给模型。',
    'decision.input.heuristicRules': '在规则回退模式下，这些信息不会发给 LLM，而是交给本地规则判断。',
    'decision.flow.title': 'LLM 怎么辅助决策',
    'decision.flow.base': '逆向设计会先生成并排序候选，AI 决策层只在结果出来后参与解读。',
    'decision.flow.llm.context': '后端会把需求、候选和约束整理成结构化上下文，发送给决策模型。',
    'decision.flow.llm.output':
      '模型必须返回结构化 JSON：headline、summary、recommendedCandidateId、rationale、risks 和 nextAction。',
    'decision.flow.fallback.trigger': '如果没有 API Key、模型调用失败或返回内容不合法，就改用规则回退。',
    'decision.flow.fallback.rules': '规则回退主要参考 DeltaE、Manufacturability、Process drift 以及约束状态。',
    'decision.selected': 'AI 推荐优先评审',
    'common.loading': '加载中',
  ...
  en: {
    ...
    'home.form.submittingTitle': 'Computing candidate options',
    'home.form.submittingBody':
      'This request finishes candidate generation, physics simulation, and decision summarization before the response returns. Please wait and avoid submitting twice.',
    'home.form.submittingHint': 'The result page opens automatically once the response is ready.',
    'home.form.submittingStep.generate': 'Generate candidate film stacks',
    'home.form.submittingStep.simulate': 'Run thin-film simulation and local refinement',
    'home.form.submittingStep.decision': 'Assemble AI decision support',
    ...
    'workspace.loadingTitle': 'Loading run results',
    'workspace.loadingNote': 'Syncing candidate options, AI decision support, and artifacts.',
    'decision.mode.llm': 'LLM assisted',
    'decision.mode.heuristic': 'Heuristic fallback',
    'decision.confidence.high': 'High confidence',
    'decision.confidence.medium': 'Medium confidence',
    'decision.confidence.low': 'Low confidence',
    'decision.explainer.title': 'What The AI Decision Layer Does',
    'decision.explainer.body':
      'The current AI decision layer turns computed results into a reviewable recommendation. It does not replace candidate generation or physics simulation.',
    'decision.role.recommend': 'It highlights which option should be reviewed first: {candidateId}.',
    'decision.role.scope': 'Its outputs are the decision headline, summary, rationale, risks, and next action.',
    'decision.role.limit': 'It does not train the model or calculate film thickness directly. Those remain in inverse design and thin-film simulation.',
    'decision.role.llm': 'LLM assistance is active with model {model}.',
    'decision.role.fallback': 'No usable LLM response was used, so the system fell back to local heuristics.',
    'decision.inputs.title': 'Inputs Used By The Decision Layer',
    'decision.input.request': 'The base inputs are the requirement, target color, incidence angle, and polarization.',
    'decision.input.candidates': 'It also references the top {count} candidate options with parameters, metrics, and rationale.',
    'decision.input.constraints': 'It also references up to {count} manufacturing constraints or watchouts.',
    'decision.input.llmPrompt': 'In LLM mode, this information is packaged into structured context and sent to the model.',
    'decision.input.heuristicRules': 'In heuristic fallback mode, this information stays local and is evaluated by rule-based logic.',
    'decision.flow.title': 'How LLM Assistance Works',
    'decision.flow.base': 'Inverse design generates and ranks candidates first. The AI decision layer only interprets the finished result set.',
    'decision.flow.llm.context': 'The backend packages the brief, ranked candidates, and constraints into structured context for the decision model.',
    'decision.flow.llm.output':
      'The model must return structured JSON with headline, summary, recommendedCandidateId, rationale, risks, and nextAction.',
    'decision.flow.fallback.trigger': 'If there is no API key, the call fails, or the response is invalid, the system falls back to heuristics.',
    'decision.flow.fallback.rules':
      'The fallback mainly evaluates DeltaE, manufacturability, process drift, and constraint state.',
    'decision.selected': 'AI-selected review target',
